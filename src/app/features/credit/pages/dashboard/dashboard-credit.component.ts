...
import {
  LucideAngularModule,
  Award, CheckCircle, CheckSquare, XOctagon,
  Search, FileText, LayoutDashboard, Globe,
  MapPin, Building2, ChevronRight, BarChart2,
} from 'lucide-angular';
import {
  CardComponent,
...
export class DashboardCreditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.creditService.getTbByProd().subscribe({
      next: (data) => { this.tbProduits.set(data); this.tbIsLoading.set(false); },
      error: () => { this.tbIsLoading.set(false); },
    });
  }
  ...
  selectRegion(id: number) {
    ...
    this.creditService.getZonesByRegion(id).subscribe({
      next: (zones) => { this.listeZones.set(zones); this.isLoadingZones.set(false); },
  ...
  rechercherTb() {
    ...
    this.creditService.getTbByProdFiltre(
      this.tbFilterAgence() ?? '',
      this.tbFilterDateDebut(),
      this.tbFilterDateFin(),
    ).subscribe({
      next: (data) => { this.tbProduits.set(data); this.tbIsSearching.set(false); },
      error: () => {
