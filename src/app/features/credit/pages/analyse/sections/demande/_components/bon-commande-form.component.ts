            ...
            @for (b of bons(); track $index) {
              <div class="rounded-lg border border-border bg-muted/20 p-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-0">
                  ...
                  @if (b.dateBonCmde) {
                    <div class="flex justify-between py-1.5 border-b border-border/40">
                      <span class="text-xs text-muted-foreground">Date d'expiration de piece</span>
                      <span class="text-xs font-medium text-foreground">{{
                        b.dateBonCmde | date: 'dd/MM/yyyy'
                      }}</span>
                    </div>
                  }
                  @if (b.mtHt != null) {
                    <div class="flex justify-between py-1.5 border-b border-border/40">
                      ...
                      <span class="text-xs font-medium text-foreground"
                        >{{ b.mtHt | number: '1.0-0' : 'fr-FR' }} FCFA</span
                      >
                    </div>
                  }
                  @if (b.mtTtc != null) {
                    <div class="flex justify-between py-1.5 border-b border-border/40">
                      ...
                      <span class="text-xs font-medium text-foreground"
                        >{{ b.mtTtc | number: '1.0-0' : 'fr-FR' }} FCFA</span
                      >
                    </div>
                  }
                  @if (b.adresse) {
        ...
        @if (showForm()) {
          <form [formGroup]="form" (ngSubmit)="save()" class="flex flex-col gap-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <app-form-input name="numBonCmde" label="N° Bon de commande" type="text" required />
              <app-form-input
                name="entreprise"
                label="Commanditaire(Entreprise)"
                type="text"
                required
              />
              <app-form-input
                name="dateBonCmde"
                label="Date d'expiration piece"
                type="date"
                required
              />
              <app-form-input
                name="adresse"
                label="Localisation de l'entreprise (Ville/Commune)"
                type="text"
                required
              />
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <app-form-input name="cel" label="Cellulaire" type="text" />
        ...
        @if (bons().length === 0 && !showForm()) {
          <div class="flex flex-col items-center justify-center py-10 text-center">
            <lucide-icon
              ...
              [size]="28"
              class="text-muted-foreground/40 mb-2"
            />
...
export class BonCommandeFormComponent {
  ...
  openEdit(b: CreditBonDeCommande) {
    ...
    this.form.reset({
      numBonCmde: b.numBonCmde ?? null,
      entreprise: b.entreprise ?? null,
      dateBonCmde: b.dateBonCmde ?? null,
