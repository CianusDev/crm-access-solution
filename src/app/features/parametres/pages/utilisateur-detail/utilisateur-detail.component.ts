...
export class UtilisateurDetailComponent {
  ...
  readonly showPermissions = computed(
    () => this.connectedRole() === 'Admin' || this.connectedRole() === 'CHARGE_COMIT',
  );
  ...
  private patchInfoForm(u: Utilisateur) {
    this.infoForm = {
      ...
      phone: u.phone ?? '',
      flotte: u.flotte ?? '',
      profil: u.profil?.id ?? 0,
      service: u.service?.id ?? 0,
      agence: u.agence?.id ?? 0,
      commune: u.commune?.id ?? 0,
      sexe: u.sexe ?? '',
      dateNaissance: u.dateNaiss ? u.dateNaiss.slice(0, 10) : '',
      fonction: u.fonction ?? '',
      numPiece: u.cni ?? '',
      typPiece: u.typPiece ?? '',
      situationMatri: u.situation ?? '',
  ...
  private loadReferentiels(u: Utilisateur) {
    this.service.getFormData().subscribe({
      ...
      error: (err) => {
        console.error('[UtilisateurDetail] formData error=', err);
        this.toast.error('Erreur référentiels.');
        this.isLoading.set(false);
      },
    });
  }
    ...
    this.service.getTypesCredit().subscribe({
      next: (types) => {
        this.service.getPermissionsTypeCredit(userId).subscribe({
          ...
          error: () => {
            this.permissions.set(
              types.map((t) => ({ typeCredit: t.id, libelle: t.libelle, type: '0' })),
            );
          },
        });
      },
  ...
  changerStatut() {
    ...
    this.service.changerStatut({ user: u.id }).subscribe({
      next: () => {
        this.toast.success('Statut modifié.');
        this.isSaving.set(false);
        this.loadUser(u.id);
      },
      error: () => {
  ...
  changerMotPasse() {
    ...
    if (this.isAdmin()) {
      ...
      this.service
        .changerMotPasseAdmin({ utilisateur: u.id, password: pwd, passwordConfirm: pwd })
        .subscribe({
          next: () => {
            this.toast.success(`Nouveau mot de passe : ${pwd}`);
            this.isSaving.set(false);
          },
          error: () => {
            this.toast.error('Erreur.');
      ...
      this.service.changerMotPasseUser(this.pwdForm).subscribe({
        next: () => {
          ...
          this.isSaving.set(false);
          this.pwdForm = { old_password: '', password: '', password_confirmation: '' };
        },
        error: () => {
          this.toast.error('Erreur.');
          this.isSaving.set(false);
  ...
  updateCode() {
    ...
    this.service.updateCode({ user: u.id, code: this.codeForm.code }).subscribe({
      next: () => {
        this.toast.success('Code mis à jour.');
        this.isSaving.set(false);
      },
      error: () => {
        this.toast.error('Erreur.');
  ...
  saveZone() {
    ...
    this.service.affecterZone({ user: u.id, zone: z }).subscribe({
      next: () => {
        this.toast.success('Zone affectée.');
        this.isSaving.set(false);
      },
      error: () => {
        this.toast.error('Erreur.');
  ...
  goBack() {
    this.router.navigate(['/app/parametres/utilisateurs']);
  }
