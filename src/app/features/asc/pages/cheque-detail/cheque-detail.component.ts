...
import {
  ...
  FileText, Eye, CheckSquare, Square, Plus, Edit2, Trash2,
  Send, Loader, Printer,
} from 'lucide-angular';
import {
  CardComponent,
...
export class ChequeDetailComponent implements OnInit {
  ...
  openEditForm(d: AscDemande) {
    ...
    if (current) {
      this.cheque.set({ ...current, demandes: (current.demandes ?? []).filter((x) => x.id !== d.id) });
    }
  ...
  saveDemande() {
    ...
    if (this.isEditMode()) {
      ...
      this.ascService.updateSousDemande(id, { numcheque: this.numcheque(), montantSollicite: montant }).subscribe({
        next: (res) => {
          if (res.status === 200) {
            this.showAddForm.set(false);
            this.isEditMode.set(false);
        ...
        error: () => { this.toast.error('Erreur lors de la modification.'); this.isSaving.set(false); },
      });
    } else {
      this.ascService.saveSousDemande({ numcheque: this.numcheque(), montantSollicite: montant }).subscribe({
        next: (res) => {
          if (res.status === 200) {
            this.showAddForm.set(false);
            this.montantSollicite.set('');
        ...
        error: () => { this.toast.error('Erreur lors de l\'enregistrement.'); this.isSaving.set(false); },
      });
    }
  }
  ...
  deleteDemande(d: AscDemande) {
    ...
    this.ascService.deleteSousDemande({ avCheque: d.id }).subscribe({
      ...
      error: () => { this.toast.error('Erreur lors de la suppression.'); this.deletingId.set(null); },
    });
  }
  ...
  openNumModal(d: AscDemande, mode: 'add' | 'edit') {
    ...
    this.numModalTitle.set(mode === 'add' ? 'Ajout numéro de la demande' : 'Modification du numéro de la demande');
    this.numModalBtn.set(mode === 'add' ? 'Ajouter' : 'Modifier');
    this.showNumModal.set(true);
  }

  closeNumModal() { this.showNumModal.set(false); this.numDemandeInput.set(''); }

  saveNumDemande() {
    const d = this.numModalDemande();
    if (!d) return;
    this.isSaving.set(true);
    this.ascService.updateSousDemande(d.id, {
      numcheque: this.numcheque(),
      montantSollicite: d.montantSollicite,
      numDemandeAsc: this.numDemandeInput(),
    }).subscribe({
      next: (res) => {
        if (res.status === 200) { this.closeNumModal(); this.reload(); }
        else { this.toast.error(res.message ?? 'Erreur.'); this.isSaving.set(false); }
      },
  ...
  closeSubmitModal() { this.showSubmitModal.set(false); this.submitObs.set(''); }
  ...
  confirmSubmit() {
    ...
    this.ascService.sendDecision({ idAsc: d.id, decision: 1, observation: obs }).subscribe({
      ...
      error: () => { this.toast.error('Erreur lors de l\'envoi.'); this.isSubmitting.set(false); },
    });
  }
      ...
      content: [
        {
          columns: [
            ...
            [{
              style: 'entete',
              table: { body: [[{ columns: [{ text: "FICHE DE DEMANDE D'AVANCE SUR CHEQUE", style: 'enteteText' }] }]] },
            }],
          ],
          margin: [0, 0, 0, 20],
        },
        {
          columns: [
            [{ text: [{ text: '\n\nTel : ', style: 'styleCle' }, { text: '+225 21 22 21 50 / +225 05 94 27 67 05', style: 'styleValeur' }] }],
            [{
              margin: [45, 0, 0, 0],
              text: [
                { text: 'Imprimé le : ', style: 'styleCle' }, { text: new Date().toLocaleString(), style: 'styleValeur' },
                { text: '\n\nImprimé par : ', style: 'styleCle' }, { text: `${user?.nom ?? ''} ${user?.prenom ?? ''}`, style: 'styleValeur' },
              ],
            }],
          ],
          margin: [0, 0, 0, 25],
        },
        {
          ...
          margin: [0, 0, 0, 10],
          columns: [
            { text: [{ text: 'Numéro de la demande:\n', style: 'styleCle' }, { text: d.numDemandeAsc ?? '—', style: 'styleValeur' }] },
            { text: [{ text: 'Date de la demande:\n', style: 'styleCle' }, { text: fmtDate(d.dateDemande ?? d.datedemande), style: 'styleValeur' }] },
            ...(d.dateDecaissement ? [{ text: [{ text: 'Débourser le:\n', style: 'styleCle' }, { text: fmtDate(d.dateDecaissement), style: 'styleValeur' }] }] : ['']),
          ],
        },
        {
          margin: [30, 0, 0, 10],
          columns: [
            { text: [{ text: 'Montant sollicité: ', style: 'styleCle' }, { text: fmtMontant(d.montantSollicite) + ' FCFA', bold: true, fontSize: 12 }] },
            { text: [{ text: 'Montant accordé: ', style: 'styleCle' }, { text: fmtMontant(d.montantAccorde ?? d.montantSollicite) + ' FCFA', bold: true, fontSize: 12 }] },
          ],
        },
        {
          table: {
            ...
            body: [[
              {
                border: [false, true, false, true], margin: [0, 5, 0, 20],
                stack: [
                  { text: [
                    { text: 'Raison sociale ou Désignation\n ', style: 'styleCle' }, { text: (c.client?.nomPrenom ?? '—') + '\n\n', style: 'styleValeur' },
                    ...(c.client?.typeAgent === 'SC' ? [{ text: 'Gérant\n ', style: 'styleCle' }, { text: (c.client?.gerant?.nomDirigeant ?? '—') + '\n\n', style: 'styleValeur' }] : []),
                    { text: 'Agence\n ', style: 'styleCle' }, { text: 'Agence ' + (c.client?.agence?.libelle ?? '—') + '\n\n', style: 'styleValeur' },
                  ]},
                  { columns: [
                    { text: [{ text: 'Code client\n ', style: 'styleCle' }, { text: (c.client?.codeClient ?? '—') + '\n\n', style: 'styleValeur' }] },
                    { text: [{ text: 'Type compte\n ', style: 'styleCle' }, { text: (c.client?.typeAgent === 'SC' ? 'Personne morale' : 'Personne physique') + '\n\n', style: 'styleValeur' }], alignment: 'right' },
                  ]},
                  { text: [
                    { text: 'Nombre de chèques remisés\n ', style: 'styleCle' }, { text: String(c.client?.nbreChequeRemise ?? 0) + '\n\n', style: 'styleValeur' },
                    { text: 'Incidents de paiement\n ', style: 'styleCle' }, { text: String(c.client?.nbreIncidentPaie ?? 0), style: 'styleValeur' },
                  ]},
                ],
              },
              {
                margin: [0, 5, 0, 20], border: [false, true, false, true],
                stack: [
                  { text: [
                    { text: 'Tireur\n ', style: 'styleCle' }, { text: (c.tireur ?? '—') + '\n\n', style: 'styleValeur' },
                    { text: 'Banque du Tireur\n', style: 'styleCle' }, { text: (c.banque ?? '—') + '\n\n', style: 'styleValeur' },
                  ]},
                  { columns: [
                    { text: [{ text: 'Nature de la Prestation\n ', style: 'styleCle' }, { text: (c.naturePrestation?.libelle ?? '—') + '\n\n', style: 'styleValeur' }] },
                    { text: [{ text: 'Date du chèque\n ', style: 'styleCle' }, { text: fmtDate(c.dateCheque) + '\n\n', style: 'styleValeur' }], alignment: 'right' },
                  ]},
                  { columns: [
                    { text: [{ text: 'N° Chèque\n ', style: 'styleCle' }, { text: (c.numcheque ?? '—') + '\n\n', style: 'styleValeur' }] },
                    { text: [{ text: 'N° Chèque PERFECT\n ', style: 'styleCle' }, { text: (c.numTransaction ?? '—') + '\n\n', style: 'styleValeur' }], alignment: 'right' },
                  ]},
                  { text: [{ text: 'Montant du chèque : ', style: 'styleCle' }, { text: '  ' + fmtMontant(c.montantCheque) + ' FCFA', bold: true, fontSize: 12 }] },
                ],
              },
            ]],
          },
        },
        { text: 'LISTE DES OBSERVATIONS DES UTILISATEURS', fontSize: 16, marginTop: 25, marginBottom: 20, alignment: 'center', decoration: 'underline', bold: true },
        {
          table: {
            headerRows: 1,
