...
@Component({
  ...
  template: `
    ...
    @if (isOpen()) {
      <div
        class="absolute right-0 top-full mt-2 z-50 w-80 rounded-lg border border-border bg-popover shadow-lg overflow-hidden"
        role="dialog"
        ...
        @if (notifications().length > 0) {
          <div class="border-t border-border px-4 py-3">
            ...
            >
              ...
              @if (isMarkingAll()) { En cours… } @else { Tout marquer comme lu }
            </button>
          </div>
        }
      </div>
