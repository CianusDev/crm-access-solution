...
const CORA_DASHBOARD_ROLES = [Admin, DG, DGA, D_EXPL, RESPO_EXPL, Charge_Cora, RESPO_FO, ADMIN_AUDIT, Gestion_Cora] as const;
// Demandes en attente (old: Admin, DGA, D_EXPL, RESPO_EXPL, Gestion_Cora, Agent_BO, Charge_Cora)
const CORA_PENDING_ROLES   = [Admin, DGA, D_EXPL, RESPO_EXPL, Gestion_Cora, Agent_BO, Charge_Cora] as const;
// Liste des CORAs (old: Admin, DG, DGA, D_EXPL, RESPO_EXPL, Agent_BO, RESPO_FO, Charge_Cora, ADMIN_AUDIT)
const CORA_LIST_ROLES      = [Admin, DG, DGA, D_EXPL, RESPO_EXPL, Agent_BO, RESPO_FO, Charge_Cora, ADMIN_AUDIT] as const;
...
const ASC_PENDING_ROLES   = [Admin, ADMIN_AUDIT, DGA, D_EXPL, ASSC_PME, RC, CC, RESPO_FO, AGENT_ACC, RESPO_EXPL] as const;
// Chèques en cours / 7j (old: Admin, ADMIN_AUDIT, DGA, D_EXPL, ASSC_PME, RESPO_EXPL, RESPO_FO, AGENT_ACC, RC, CC)
const ASC_CHEQUES_ROLES   = [Admin, ADMIN_AUDIT, DGA, D_EXPL, ASSC_PME, RESPO_EXPL, RESPO_FO, AGENT_ACC, RC, CC] as const;
// Liste des chèques + Tireurs (old: Admin, DG, DGA, D_EXPL, ASSC_PME, CA, RC, CC, RESPO_FO, AGENT_ACC, RESPO_EXPL, RESPO_CLT_PME)
const ASC_LIST_ROLES      = [Admin, DG, DGA, D_EXPL, ASSC_PME, CA, RC, CC, RESPO_FO, AGENT_ACC, RESPO_EXPL, RESPO_CLT_PME] as const;
...
const CREDIT_LIST_ROLES = [
  ...
  SUP_RISQ_ZONE, CHARGE_COMIT, RESPO_RGL, RESPO_JURIDIQUE, RESPO_ASSUR,
] as const;
...
export const DEFAULT_MENU: SidebarGroup[] = [
  ...
  },
  {
    ...
    icon: Ellipsis,
    items: [
      { label: 'Tableau de bord',       href: '/app/cora/dashboard',  icon: LayoutDashboard, roles: [...CORA_DASHBOARD_ROLES] },
      ...
      { label: 'Demandes en attente',   href: '/app/cora/pending',    icon: Clock,           roles: [...CORA_PENDING_ROLES] },
  ...
  },
  {
    ...
    icon: CreditCard,
    items: [
      { label: 'Tableau de bord réseau', href: '/app/credit/dashboard',        icon: LayoutDashboard, roles: [...CREDIT_DASHBOARD_SIEGE_ROLES] },
      ...
      { label: 'Liste des demandes',      href: '/app/credit/list',             icon: List,            roles: [...CREDIT_LIST_ROLES] },
      { label: 'Tirages découvert',       href: '/app/credit/tirage/list',      icon: CreditCard,      roles: [...CREDIT_TIRAGE_ROLES] },
      { label: 'Employeurs éligibles',    href: '/app/credit/employeur/list',   icon: Users,           roles: [...CREDIT_EMPLOYEUR_ROLES] },
    ],
  },
  {
    ...
    items: [
      ...
      { label: 'Demandes en attente',     href: '/app/asc/pending',         icon: Clock,           roles: [...ASC_PENDING_ROLES] },
      { label: 'Liste des chèques',       href: '/app/asc/list',            icon: List,            roles: [...ASC_LIST_ROLES] },
      { label: 'Liste des tireurs',       href: '/app/asc/tireurs',         icon: Users,           roles: [...ASC_LIST_ROLES] },
      { label: 'Chèques en attente (7j)', href: '/app/asc/cheques-attente', icon: AlarmClock,      roles: [...ASC_CHEQUES_ROLES] },
    ],
  },
  {
    label: 'Paramètres',
